...
export class Answer {
	...
	public getCustomAIContext(): string {
		...
		switch (this.problems.getWorst()) {
			case Problem.Provocation:
				output += `L'utilisateur t'a ping dans son message, ce qui t'agace car tu es solicité inutilement`;
				break;

			case Problem.Correction:
				output += `L'utilisateur à utilisé un mot contenant "théo" mais qui existe.
                Comme "théorème" par exemple. Rien de grave mais ça t'agace.`;
				break;

			case Problem.Violation:
				output += `L'utilisateur à écris "Théo" avec un H, tu va devoir l'insulter.`;
				break;

			case Problem.Trick:
				output += `L'utilisateur à essayé de dissimuler un "Théo" dans son message, mais tu l'as repéré.`;
				break;
		}

		output += this.problems.has(Problem.BadName)
			? `L'utilisateur à un "Théo" avec un h dans son prénom`
			: `L'utilisateur ne s'appelle pas "Théo"`;
	...
	public async getMessage(): Promise<string> {
		...
		if (process.env['TESTING'] == '1')
			return (
				'(testing mode enabled) no ai message, custom context = \n' +
